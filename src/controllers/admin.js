...
module.exports = {
  ...
  async single(req, res) {
    ...
    return Adminlogin
      .create({
        ...
        role: req.body.role,
        token: `${new Date().getTime()}_${Math.random().toString(36).slice(2)}${Math.floor(Math.random() * 100)}_`
      })
      .then(() => {
        res.status(201).send({
          errored: false,
  ...
  async update(req, res) {
    ...
    for (const [key, value] of Object.entries(req.body)) { // eslint-disable-line
      if (key !== 'password') query += (query.length > 1) ? `, ${key} = '${value}' ` : ` ${key} = '${value}'`;
  ...
  async logout(req, res) {
    ...
    return sequelize.query(`UPDATE Adminlogins SET token = '${newToken}' WHERE token = '${req.cookies.admin_token}'`)
      .then((rslt) => {
        res.status(200).send({
          errored: (rslt[0].affectedRows < 1),
          message: {}
  ...
  async self(req, res) {
    // fetch self data
    if (await isAuth(req.body, req.cookies.admin_token, 'any') === false
      || !req.cookies.admin_token || req.cookies.admin_token.length <= 0) {
      return res.status(400).send({
        errored: true,
        message: MSG.ERROR.UNAUTHORISED_ACCESS
      });
    ...
    return Adminlogin
      .findOne({ where: { token: req.cookies.admin_token } })
      .then((result) => {
        res.status(200).send({
          errored: !(result && result.id),
