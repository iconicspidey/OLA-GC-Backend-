...
module.exports = {
  ...
  async update(req, res) {
    ...
    return Students.update(req.body, {
  ...
  async remove(req, res) {
    ...
    return Students.destroy({ where: { id: req.params.id } })
      .then((deleteStatus) => {
        res.status(200).send({
  ...
  async getsingle(req, res) {
    ...
    return Students.findOne({ where: columns })
      .then((result) => {
        res.status(200).send({
  ...
  async getmultiple(req, res) {
    ...
    return Students.findAll({ where: req.body })
      .then((result) => {
        res.status(200).send({
  ...
  async resetPassword(req, res) {
    ...
    return Students.update({ password: req.body.password }, { where: { id: req.params.id } })
      .then(([affected]) => {
        res.status(200).send({
          errored: affected !== 1,
          message: affected === 1 ? MSG.SUCCESS.RECORD_UPDATED : MSG.ERROR.RECORD_UPDATE_FAILED,
        });
      })
      .catch((error) => {
        res.status(400).send({
  ...
  async search(req, res) {
    // search students by name substring
    if ((await isAuth(req.body, req.cookies.admin_token, 'any')) === false) {
      return res.status(400).send({ errored: true, message: MSG.ERROR.UNAUTHORISED_ACCESS });
    }
    ...
    return Students.findAll({
      where: {
        full_name: { [require('sequelize').Op.like]: `%${name}%` }
      }
    })
      .then((results) => {
        res.status(200).send({
          errored: !(results && results.length >= 1),
          message: !(results && results.length >= 1) ? MSG.ERROR.RECORD_NOT_FOUND : results
        });
      })
      .catch(() => {
        res.status(400).send({ errored: true, message: MSG.ERROR.INVALID_SEARCH });
